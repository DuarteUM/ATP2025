...
def mostrar_grafico_piramide_etaria(idades):
    """EXTRA: Gráfico com escalas curtas de 18 aos +75 anos."""
    # Definimos os intervalos (bins) com passos mais curtos conforme pedido
    bins = [18, 30, 45, 60, 75, 120]
    labels = ['18-30', '31-45', '46-60', '61-75', '+75']
    counts = np.histogram(idades, bins=bins)[0]
    
    plt.figure("Perfil Etário", figsize=(10, 5))
...
def mostrar_grafico_pizza_triagem(cores_atendidas):
    """EXTRA: Gráfico circular detalhado com as 4 cores de Manchester."""
    labels = ['Vermelha', 'Laranja', 'Amarela', 'Verde']
    # Contagem exata de cada cor
    counts = [
    ...
    # Cores oficiais da triagem médica
    colors = ['#ff0000', '#ff8c00', '#ffff00', '#00ff00']
    
    # Destacar a fatia vermelha (explode) se houver casos
    explode = (0.1, 0, 0, 0) if cores_atendidas.count('vermelha') > 0 else (0, 0, 0, 0)
    
    plt.figure("Triagem Manchester (Cores)", figsize=(8, 6))
