...
def criar_layout(tema="Claro"):
    # CORES: Navy para o título claro, Dourado para o título escuro (contraste alto)
    ...
    layout = [
        ...
        [sg.Frame("Análise de Perfil", [
            [sg.Button("Gráfico: Faixas Etárias"), sg.Button("Gráfico: Triagem de Manchester")]
        ], pad=(0,10))],
...
window = sg.Window("Simulação Clínica", criar_layout("Claro"), finalize=True)
...
while True:
    ...
    if event == "-MODO-":
        ...
        window.close()
        window = sg.Window("Simulação Clínica", criar_layout(tema_escolhido), location=loc, finalize=True)
        continue
    ...
    if event == "EXECUTAR SIMULAÇÃO":
        try:
            ...
            esp_med = formatar_tempo(np.mean(stats_fixos["espera"])) if total > 0 else "00:00:00"
            sat_media = np.mean(stats_fixos["satisfacao_individual"]) if total > 0 else 0
            
            # Construção do Relatório com a contagem das pulseiras
            output = f"{'='*40}\n ESTATÍSTICAS GERAIS\n{'='*40}\n"
            output += f" Doentes Atendidos: {total}\n"
            output += f" Tempo Médio de Espera: {esp_med} (HH:MM:SS)\n"
    ...
    # Eventos de Gráficos
    if event == "Gráfico: Evolução da Fila" and stats_fixos: plots.mostrar_grafico_fila(stats_fixos)
    if event == "Gráfico: Ocupação" and stats_fixos: plots.mostrar_grafico_ocupacao(stats_fixos)
    if event == "Gráfico: Correlação (10-30)" and correlacao_y: plots.mostrar_grafico_correlacao(correlacao_x, correlacao_y)
    if event == "Gráfico: Pirâmide Etária" and stats_fixos: plots.mostrar_grafico_piramide_etaria(stats_fixos["idades"])
    if event == "Gráfico: Pizza Triagem" and stats_fixos: plots.mostrar_grafico_pizza_triagem(stats_fixos["cores_atendidas"])
